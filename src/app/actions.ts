...
export type AddPatientFormState = {
  message: string;
  errors?: z.ZodError<typeof addPatientFormSchema>['formErrors']['fieldErrors'];
  isSuccess: boolean;
};
...
export async function addPatientAction(
  ...
  const validatedFields = addPatientFormSchema.safeParse(
  ...
  try {
    ...
    await patientRef.set({
      ...
      nextOfKin: {
        ...
        relation: data.nextOfKinRelation,
    ...
    return { message: 'Patient successfully added!', isSuccess: true };
  } catch (error) {
    console.error(error);
    const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred.';
    return { message: `Failed to add patient: ${errorMessage}`, isSuccess: false };
  }
}
